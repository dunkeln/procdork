...
from okf_flush import flush_okf
...
def observe_sql_command(
    ...
    step = step or 1
    record = record_sql_observation(observe_sql(sql, actor, session_id, step), duckdb_path)
...
@click.option("--duckdb-path", default=None, help="DuckDB/MotherDuck path. Defaults to DUCKDB_PATH or local data/harness.duckdb.")
@click.option("--actor", default="unknown", show_default=True)
def observe_sql_session_command(sql_file: Path, session_id: str | None, duckdb_path: str | None, actor: str) -> None:
    ...
    records = [
        record_sql_observation(observe_sql(statement, actor, session_id, index), duckdb_path)
...
@main.command("okf-flush")
@click.option("--marts-root", default="transforms/dbt/models/marts", show_default=True)
@click.option("--okf-root", default="okf", show_default=True)
def okf_flush_command(marts_root: str, okf_root: str) -> None:
    """Refresh OKF docs from already-promoted assets."""
    result = flush_okf(Path(marts_root), Path(okf_root))
    click.echo(result.model_dump_json(indent=2))
